...
from main_app.models import Author, Book, Artist, Song, Product, Review, DrivingLicense, Driver, Car, Registration, Owner
...
def show_all_authors_with_their_books():
    ...
    for author in authors:
        ...
        if not books:
            continue
        result += f"\n{author.name} has written - {', '.join([book.title for book in books])}!"
...
def calculate_licenses_expiration_dates():
    lics = DrivingLicense.objects.all().order_by('-license_number')
    res = ''
    for lic in lics:
        res += f"\nLicense with number: {lic.license_number} expires on {lic.issue_date + timedelta(days=365)}!"
    return res

def get_drivers_with_expired_licenses(due_date: date):
    return Driver.objects.filter(license__issue_date__lt=due_date - timedelta(365))
...
def register_car_by_owner(owner: Owner):
    ...
    if not (first_unrelated_reg and first_unreg_car):
        return 'no car or registration'

    first_unreg_car.owner = owner
    first_unreg_car.save()

    first_unrelated_reg.car = first_unreg_car
    first_unrelated_reg.registration_date = date.today()
    first_unrelated_reg.save()
    return f"Successfully registered {first_unreg_car.model} to {owner.name} with registration number {first_unrelated_reg.registration_number}."
