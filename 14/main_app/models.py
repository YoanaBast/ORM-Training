...
def letters_spaces_only_validator(value):
    if not value.strip().isalpha():
        raise ValidationError("Name can only contain letters and spaces")

def phone_validator(value):
    if not (value.startswith("+359") and len(value) == 13):
        raise ValidationError("Phone number must start with '+359' followed by 9 digits")

class Customer(models.Model):
    name = models.CharField(max_length=100, validators=[letters_spaces_only_validator])
...
class Book(BaseMedia):
    ...
    isbn = models.CharField(max_length=20, unique=True, validators=[
